...
│   ├── tag_analysis.py       「型」別の傾向の集計（記録20件以上で自動的に動く）
│   └── threads_api.py        共通部品
...
「案1を投稿したよ」と伝える（または `/posted`）だけで、`data/posted.csv` にタグ付きで
記録されます（URLは無くてもOK。本文を照合して自動で紐づきます）。

### 🧾 「型」別の傾向の自動集計

記録が20件を超えると、週次レポートに **「型」別の傾向** の表が自動で載ります。
「あるあるは伸びるのか」「架空事例つきは効くのか」などが毎週更新されます。

読むときのポイント:

- **中央値（真ん中の値）を見てください。** 平均は1本の大当たりに釣り上げられます。
  実際に「ハッシュタグあり」は平均206でしたが、中央値は27でした（1本だけ573閲覧だったため）
- 差が小さいときは「**意味のある差は出ていません**」と表示されます。数字の上下だけを見て
  ムダに悩まないための仕組みです
- 件数が3件に届かない項目は、たまたまの結果になるので表示されません
